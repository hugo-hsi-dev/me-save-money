...
import { k, f as f2, l, m, h } from './chunks/event-state.js';
import { b, c as c$1 } from './chunks/paths.js';
import { c as c2, g, p, r } from './chunks/shared.js';
...
function prerender(validate_or_fn, fn_or_options, maybe_options) {
	...
	const wrapper = (arg) => {
		const promise = (async () => {
			const event = k();
			const state = f2(event);
			const payload = l(arg, state.transport);
			const id = __.id;
			const url = `${b}/${c$1}/remote/${id}${payload ? `/${payload}` : ''}`;
			if (!state.prerendering && !B && !event.isRemoteRequest) {
				try {
					return await g(id, arg, event, async () => {
						...
						(state.remote_data ??= {})[m(id, payload)] = prerendered.result;
			...
			const promise2 = g(id, arg, event, () => r(event, false, arg, validate, fn));
			if (state.prerendering) {
				state.prerendering.remote_responses.set(url, promise2);
			}
			const result = await promise2;
			if (state.prerendering) {
				const body = { type: 'result', result: h(result, state.transport) };
...
export { c as command, f as form, prerender, q as query };
